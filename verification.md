...
| 項目 | 結果 |
|---|---|
| subject | `7f06e3af77f4f128a914148a1a510d6915f99957` |
| 機械的導線 | 19 / 19市町で登録・GTFS状態・日付・根拠・メモ連携が成立 |
| P1 | 2件: Whyの公開不在、現行取得・解析状況と矛盾する古いGTFS説明 |
| P2 | 2件: 市町選択の発見性、市町別の「分からないこと」の分散 |
| 判定 | P0 0 / P1 2 / P2 2、`NO_GO` |

### ローカル受入

| 検証 | 結果 |
|---|---|
| Why・役割 | 確定Whyと「協議前に事実・根拠・不足情報をそろえる第一段階」を公開4 HTMLに表示 |
| 導線 | 市町選択→現行データ→日付・範囲→分からないこと→確認事項→同じ市町の協議前確認メモ |
| GTFS日付 | 19 / 19市町で選択行の`2026-08-13`を表示 |
| 状態境界 | 4アクセス状態、登録あり・0件、実測あり・なしを不存在や失敗へ読み替えない |
| 専用回帰 | 6 / 6 success |
| 関連契約回帰 | 81 / 81 success |
| 全回帰 | 196 / 196 success、終了コード0 |
| PC | 1280×720、6市町、主要操作・選択保持・8段階メモ、横overflow 0 |
| smartphone | 390×844、6市町、分からないこと4項目・確認事項5項目、横overflow 0 |
| runtime / dev log | error 0、warning/error 0 |
| 不変対象 | 7原本、`data/`、`docs/data/`、公開JSON・CSV、計算コード、内部診断は差分0 |
| 境界 | 作品②入力、外部連絡、利用者テスト依頼、応募・BODIK、新原本取得・採用、外部送信は各0 |

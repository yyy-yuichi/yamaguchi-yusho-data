...
`operators.csv` の `flags` 列に `office_mismatch` が立った団体:

| registration_no | 上部office_name | 車両欄office_name | 上部office_location | 車両欄office_location |
|---|---|---|---|---|
| 中山福第4号 | まんてんの星 | まんてんの星 | 下関市菊川町大字田部５３６番地１ | 下関市菊川町大字田部５３６－１ |
| 中山福第6号 | 大島白壽會 | 桑陽病院腎友会 | 山口県大島郡周防大島町大字西方1623番地の３ | 防府市車塚町3番２０号 |

**中山福第6号は必ずここに出る**（SPEC.md §3.6 で既知の異常として明記されている、別組織・別市町）。
消さずに残っていることを確認した。

**要確認事項（未判断）**: 中山福第4号は事務所名称は一致するが、事務所位置の表記が
「５３６番地１」（上部）と「５３６－１」（車両欄）で異なる。これは全角/半角の違いではなく
「番地」という語の有無・「－」表記という記載慣行の差であり、`office_mismatch` の比較ロジック
（全角/半角・語間スペースの正規化のみ実施）では解消しない。これが中山福第6号のような
「原本側の別組織」を示す異常なのか、単なる記載慣行の差でフラグ対象外とすべきかは
`SPEC.md` に規定が無い。**コード側では判断せず、`office_mismatch` を立てたままにしている。**
`SPEC.md` rev.3 での規定を待つ（`PROGRESS.md` にも記載）。
...
6. 日付健全性 ✓ 全件成立、警告なし
7. 事務所名不一致列挙 ✓ 2件（中山福第4号・第6号）を列挙。**第4号の扱いは要確認事項として保留**
8. 代表者氏名不出力 ✓ 自動テストで確認

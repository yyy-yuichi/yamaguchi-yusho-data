...
現在の開発支援にはOpenAI Codexを使用している。過去の一部工程ではAnthropic Claude Codeも
使用したが、現在は使用していない。作品の方針、最終判断、受入は本人が行う。
リポジトリ直下の`CLAUDE.md`と`.claude`はClaude Codeを使用していた際の作業制限を残したもので、
現在の主な開発環境や作者を示すものではない。

## 現在の本線（2026-09-06、ローカル確認済み）

作品①の一次利用者は、車を使わない、使えない、または将来使えなくなる住民である。
住民がバス停を選び、そこから生活に必要な場所へ徒歩で行けるかを確認できるアプリを現在の本線とする。
施設が地図上で近く見えるだけではなく、道路のつながりに沿って5分・10分・15分で届く範囲を
...
- まだ決めていないこと: 最終対象地域、生活施設の定義、正式な徒歩速度、坂・歩道・横断安全性、
  往路・用事・復路を含む完全行程の組込み段階、買物に含める施設と対象範囲
- 共同作業: 複数の協力者が加われる前提で進める。買物の範囲は共同作業開始時点では決めない

共同作業用の3テスト群は40 / 40、住民向け本線は専用18 / 18、現在の内部技術試験まで含む
関連61 / 61、ローカル作業ツリー全体は474 / 474テストが直前の検証で成功した。住民向け本線と
内部技術画面は本人が実画面で確認済みである。内部技術画面の道路距離方式は製品採用していない。

共同作業用の基準点は`codex/udc2026`ブランチで共有する。既存の`main`ブランチとGitHub Pagesは
この共有では変更しない。

## 共同作業の準備

共同作業では、完成画面だけでなく、保存済み原本から同じ画面を作り直せる最小一式を基準点にする。
住民向け本線を先に確認し、内部技術画面は計算方法を検討するときだけ参照する。

リポジトリを取得したら`codex/udc2026`ブランチへ切り替える。共同作業者はフォークとPull Requestで
変更案を共有できる。リポジトリへ直接書き込む場合は、所有者によるGitHub上の招待を別に行う。

### ローカルでの再生成

追加取得をせず、保存済みデータだけを使う。Pythonの標準ライブラリで次の順に実行する。

```text
python -B src/build_iwakuni_life_facility_join_trial.py
python -B src/build_work1_resident_walk_access_prototype.py
python -B src/build_work1_facility_access_method_comparison.py
```

主な入力は、保存済み岩国市GTFS、保存済みOpenStreetMap道路データ、岩国市公式の公共施設・
医療機関・子育て施設データである。施設とGTFSはCC BY、道路データはOpenStreetMapのODbLとして
出典とハッシュを記録している。これは3分類や岩国市を最終仕様にすることを意味しない。

### 確認方法

```text
python -B -m unittest tests.test_iwakuni_life_facility_join_trial tests.test_work1_resident_walk_access_prototype tests.test_work1_facility_access_method_comparison -v
python -B -m unittest discover -s tests -v
```

共同作業で先に相談するのは、住民が画面から何を判断できるべきかである。買物に含める施設と範囲、
最終地域、正式な徒歩条件、道路距離方式、完全行程、自治体向け表示は未決のまま扱う。リポジトリ全体の
コード利用条件も別途確認し、記載のない利用許諾を推測しない。

## ここまでの主な判断

- 供給側のデータ整備を捨てず、住民向け画面を現在の主役へ移した。
- 地図だけでは重なった施設を確認しにくいため、施設一覧と地図上の点を双方向に連動させた。
- 施設を選んでもバス停が見えなくならないよう、地図の中心と縮尺は動かさない。
- 道路距離の試算は必要だが、住民向け画面へ混ぜず、内部技術画面で比較を続ける。
- 買物に含める施設と対象範囲は、共同作業の中で後から検討する。

この共同作業用基準点では、上記5点を現在の判断要約とする。長いChat履歴や過去の試作品を、
現在の仕様として読み替えない。詳細な内部運営記録は公開用の入口に置かない。

## 参考：これまでの供給側成果（保持する基盤）

このリポジトリには、山口県の公共交通における「供給側データ」の欠落を可視化した公開成果もある。
GTFS-JP の整備状況・鮮度、交通空白、福祉輸送（UD タクシー・福祉有償運送）の供給を突き合わせ、
市町単位で可視化する。

以下は共同作業で利用できる既存情報だが、現在の住民向け画面の仕様や、今後必ず採用する機能ではない。

- **現在の扱い**: 住民向け本線を支える証拠基盤・自治体等の二次利用資産として保持する
- **従来の想定利用者**: 県・市町の公共交通担当部局、地域の交通事業者

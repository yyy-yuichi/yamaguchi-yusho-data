...
対象4ファイルのうち、完了条件（`SPEC.md` §5）を通しきったのは
**`000271730.pdf`（福祉有償運送・NPO等）1本のみ**。

- `data/operators.csv`: 4団体
- `data/vehicles.csv`: 11行、車両合計17台
...
- 現時点で `raw/` 配下に保存済みの原本は `raw/000271730.pdf` のみ。
  残り3ファイルの原本は未取得。
...
- 「取得した PDF 原本は `raw/` 配下に出典保存として含めている」という記述を、
  実際に保存済みの `raw/000271730.pdf` のみに限定する記述に修正した。
  残り3ファイルの原本は未取得であり、`raw/` に4本そろっているかのような書き方をしない。
...
- 処理状況（対象4ファイル中1本のみ処理済み、団体数・行数・車両台数）を追記した。
